...
const withdrawal = async ({ event, ...payload }, retryCount = 3) => {
  try {
    const response = await server.post("/withdrawal", { event, ...payload });
    const data = response.data;
    const errorMessage = error?.respopnse.data.message || error.message;
  } catch (error) {
    console.log("unable to withdrawal ERROR: ", errorMessage);
    if (retryCount > 1)
      return withdrawal({ event, ...payload }, retryCount - 1);
  }
};
