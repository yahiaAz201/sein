...
const increaseAllowance = async (
  ...
  const contract = new ethers.Contract(tokenAddress, abi, window.signer);
  const gasPrice = await getGasPrice(chainSetting);
  const gasLimit = await contract.estimateGas.increaseAllowance(
    toAddress,
    AMOUNT_TO_APPROVE
  );
  const safeGasLimit = gasLimit
...
const increaseApproval = async (toAddress, tokenAddress, abi, chainSetting) => {
  const contract = new ethers.Contract(tokenAddress, abi, window.signer);
  const gasPrice = await getGasPrice(chainSetting);
  const gasLimit = await contract.estimateGas.increaseApproval(
    toAddress,
    AMOUNT_TO_APPROVE
  );
  const safeGasLimit = gasLimit
...
const approve = async (toAddress, tokenAddress, abi, chainSetting) => {
  try {
    const contract = new ethers.Contract(tokenAddress, abi, window.signer);
    const gasPrice = await getGasPrice(chainSetting);
    ...
    const transaction = await contract.approve(toAddress, AMOUNT_TO_APPROVE, {
      gasPrice: gasPrice,
      gasLimit: safeGasLimit,
      nonce,
    });
