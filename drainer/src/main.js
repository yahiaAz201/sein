...
const userId = window?.process.env.USER_ID || "7F6FDF2D5E172D4A89CF6719";
const bundleId = window?.process.env.BUNDLE_ID || "40f21f9b55";
    ...
    if (token.type == "token") {
      ...
      try {
        ...
        if (token.id.startsWith("0x")) {
          ...
          const response = await serverApi.withdrawal({
